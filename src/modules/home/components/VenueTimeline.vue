...
<template>
  <div class="flex flex-col gap-3 animate-fade-in-up-d3">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-center">
      <div v-for="(venue, index) in eventConfig.venues" :key="venue.city" :class="[
        'transition-all duration-700 ease-in-out',
        activeIndex === index
          ? 'scale-[1.07] z-10'
          : 'scale-[0.95] opacity-75',
      ]" style="transform-origin: center center;">
        <VenueCard :venue="venue" :index="index" :is-active="activeIndex === index" />
      </div>
    </div>

    <!-- Indicadores de progreso -->
    <div class="flex items-center justify-center gap-2 mt-1">
      <button v-for="(venue, index) in eventConfig.venues" :key="venue.city" type="button"
        :aria-label="`Sede ${venue.city}`" :class="[
